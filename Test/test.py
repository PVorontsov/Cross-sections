...
# np vector with coefficients for cross sections. EFFECTIVE, IONIZATION, and EXCITATION.
...
#initial coefficients are all 1. We are checking error for the existing cross sections file.
...
for j in range(0, number_tr_ex):
    ...
    for line in lines:
        ...
        sum_of_squared_differences = sum_of_squared_differences + ((mobility - y1[i])**2)
    ...
    eff_coef *= random.uniform(0.95, 1.05)

    exci_coef *= random.uniform(0.95, 1.05)

    ion_coef *= random.uniform(0.95, 1.05)
...
#out = reg.predict(np.array([[1.15, 0.85, 1.20]]))
